...
- [Assim garantimos que $0 \leq P \leq 1$.]{.fragment}
...
::: {style="font-size: 68%;"}
A probabilidade cresce **lentamente** para $P/I$ baixo, **rapidamente** para valores intermediários, e **satura em 1** para valores elevados.
:::
...
::: {style="font-size: 82%;"}
- O coeficiente $\beta_1$ representa a diferença no valor de $z$ associada ao aumento de $X_1$ em uma unidade, mantendo $X_2, \ldots, X_k$ constantes.
...
::: {.fragment}
→ O mesmo incremento em $X_1$ pode gerar **efeitos diferentes** conforme o ponto da curva.
:::
:::
...
- **Eficiente:** menor variância assintótica entre estimadores consistentes (Cramér-Rao).
:::
:::
...
$$\hat{V}(\hat{\beta}) = H^{-1}$$
onde $H$ é a matriz Hessiana da log-verossimilhança avaliada em $\hat{\beta}$.
...
::: {.fragment}
- **Testes de hipótese:**
  - Teste $z$ individual: $z = \hat{\beta}_j\,/\,SE(\hat{\beta}_j)$
  - Teste de razão de verossimilhança (para hipóteses conjuntas)
:::
:::
...
::: {.fragment}
- **Diferença estimada** (colunas 1–3, candidato médio):

| Modelo | Diferença preto − branco |
|--------|--------------------------|
| LPM | +8,4 p.p. |
| Logit | +6,0 p.p. |
| Probit | +7,1 p.p. |
